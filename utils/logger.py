...
class Logger:
    ...
    def _dbg(self, message, **kwargs):
        ...
        if self.format == "vertical":
            ...
            output += f" {self.separator}{Fore.LIGHTWHITE_EX} {f' {Fore.LIGHTBLACK_EX}●{Fore.LIGHTWHITE_EX} '.join([f'{key} -> {value}' for key, value in kwargs.items()])}"
        print(output)
    ...
    def _inp(self, message, integer):
        timestamp = self.ctime.time()
        output = f"{timestamp} \x1b[38;2;238;130;238mINP {self.separator}{Fore.LIGHTWHITE_EX}{message}\n"
        output += f"{' ' * 8} \x1b[38;2;238;130;238m└ {Fore.LIGHTWHITE_EX}> {Fore.RESET}"
        if integer:
            while True:
                try:
